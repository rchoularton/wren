 ...
 * create-wren
 *
 * Interactive scaffolder for Wren. Run as an npm initializer:
 *
...
function yamlList(items) {
  ...
  return '\n' + items.map((i) => `    - ${yamlDq(i)}`).join('\n');
}

// Encode an arbitrary string as a double-quoted YAML scalar: collapse newlines
// to spaces, escape backslash and double-quote. Keeps user answers (name,
// project, journal, path with spaces, etc.) from producing invalid YAML.
function yamlDq(value) {
...
async function main() {
  ...
  console.log(`\n  Copying Wren into ${relative(process.cwd(), root) || '.'} …`);
  ...
  // Initialise a git repo so the project is version-controlled from the start,
  // and so any tool that consults `.gitignore` (e.g. the scrub check) behaves
  // correctly. Tolerate git being absent — it must never block setup.
  try {
    execFileSync('git', ['init', '--quiet'], { cwd: root, stdio: 'ignore' });
  } catch {
    /* git not installed — fine, the project just isn't a repo yet */
  }
  ...
  const config = `# Generated by create-wren. Edit freely; re-run ./setup.sh to apply.
identity:
  author_name: ${yamlDq(authorName)}
  orcid: ${yamlDq(orcid)}
  email: ${yamlDq(email)}
  coauthors: []

project:
  name: ${yamlDq(projectName)}
  namespace: ${yamlDq(namespace)}
  discipline: ${yamlDq(discipline)}
  root: ${yamlDq(root)}
